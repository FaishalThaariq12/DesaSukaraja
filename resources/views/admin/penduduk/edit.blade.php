...
<div class="max-w-5xl mx-auto bg-white p-10 rounded-2xl shadow-lg mt-10 border border-slate-100">
  <div class="flex items-center justify-between mb-8">
    <h2 class="text-3xl font-bold text-slate-800">Edit Data Penduduk</h2>
  ...
  <form action="{{ route('admin.infografis.update', $penduduk->id) }}" method="POST" class="space-y-8">
    @csrf
    @method('PUT')
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          ...
          <input type="text" name="dusun" value="{{ $penduduk->dusun }}"
            class="w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-emerald-400 focus:outline-none" required>
        </div>
        <div>
          <label class="block mb-2 font-medium text-slate-700">Periode Bulan</label>
          <input type="text" name="bulan" value="{{ $penduduk->bulan }}"
            class="w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-emerald-400 focus:outline-none" placeholder="cth: Agustus" required>
        </div>
        <div>
          <label class="block mb-2 font-medium text-slate-700">Tahun</label>
          <input type="number" name="tahun" value="{{ $penduduk->tahun }}"
            class="w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-emerald-400 focus:outline-none" required>
        </div>
      </div>
    </div>
